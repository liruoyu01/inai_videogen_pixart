...
from model.vae_modules.resnet import Upsample2D, ResnetBlock2D
from model.vae_modules.temporal_layers import TemporalConvBlock
# from model.vae_modules.recurrent_blocks import ConvRecurrentBlock, ConvAttenRecurrentBlock
# from model.vae_modules.recurrent_blocks import SwinNewAttenRecurrentBlock as SwinAttenRecurrentBlock
...
class UpDecoderBlock3D(nn.Module):
    ...
    def _set_partial_grad(self):
        ...
        if self.add_temp_upsample:
            self.temp_conv_up.requires_grad_(True)

    def forward(self, hidden_states, temb=None, num_frames=1, temp_compress=False):
        for resnet, temp_conv in zip(self.resnets, self.temp_convs):
            hidden_states = resnet(hidden_states, temb=temb)
            if num_frames > 1:
                hidden_states = temp_conv(hidden_states, num_frames=num_frames)

        if self.add_temp_upsample and temp_compress:
             hidden_states = self.temp_conv_up(hidden_states, num_frames=num_frames)
