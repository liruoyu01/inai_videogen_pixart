...
from .downencoder_block_compress import DownEncoderBlock2D
from .unet_midblock import UNetMidBlock3DConv
from .updecoder_block_compress import UpDecoderBlock3D
...
class Encoder(nn.Module):
    ...
    def forward(self, x, num_frames, latent_embeds=None, temp_compress=False):
        '''
            x: (b f), c, h, w
        '''
        sample = x
        ...
        if self.training and self.gradient_checkpointing:
            ...
            if is_torch_version(">=", "1.11.0"):
                ...
                for b_id, down_block in enumerate(self.down_blocks):
                    ...
                    # )
                    sample = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(down_block), sample, num_frames, temp_compress, use_reentrant=False
                    )
                    if self.blocks_temp_li[b_id] and num_frames > 1 and temp_compress:
                ...
                for b_id, down_block in enumerate(self.down_blocks):
                    sample = torch.utils.checkpoint.checkpoint(create_custom_forward(down_block), sample, num_frames, temp_compress)
                    if self.blocks_temp_li[b_id] and num_frames > 1 and temp_compress:
                        num_frames = int(num_frames//2)
                # middle
            ...
            # down
            for b_id, down_block in enumerate(self.down_blocks):
                sample = down_block(sample, num_frames=num_frames, temp_compress=temp_compress)
                if self.blocks_temp_li[b_id] and num_frames > 1 and temp_compress:
                    num_frames = int(num_frames//2)
...
class Decoder3D(nn.Module):
    ...
    def forward(self, z, num_frames=1, latent_embeds=None, temp_compress=False):
        sample = z
        sample = self.conv_in(sample)
        ...
        if self.training and self.gradient_checkpointing:
            ...
            if is_torch_version(">=", "1.11.0"):
                ...
                for b_id, up_block in enumerate(self.up_blocks):
                    sample = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(up_block), sample, latent_embeds, num_frames, temp_compress, use_reentrant=False
                    )
                    if self.blocks_temp_li[b_id] and num_frames > 1 and temp_compress:
                        num_frames = int(num_frames*2)
            else:
                ...
                # for up_block in self.up_blocks:
                for b_id, up_block in enumerate(self.up_blocks):
                    sample = torch.utils.checkpoint.checkpoint(create_custom_forward(up_block), sample, latent_embeds, num_frames, temp_compress)
                    if self.blocks_temp_li[b_id] and num_frames > 1 and temp_compress:
                        num_frames = int(num_frames*2)
        else:
            # middle
            ...
            # for up_block in self.up_blocks:
            for b_id, up_block in enumerate(self.up_blocks):
                sample = up_block(sample, latent_embeds, num_frames=num_frames, temp_compress=temp_compress)
                if self.blocks_temp_li[b_id] and num_frames > 1 and temp_compress:
                    num_frames = int(num_frames*2)
