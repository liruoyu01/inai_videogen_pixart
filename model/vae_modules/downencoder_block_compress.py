...
class DownEncoderBlock2D(nn.Module):
    ...
    def _set_partial_grad(self):
        ...
        if self.add_temp_downsample:
            self.temp_convs_down.requires_grad_(True)

    # def forward(self, hidden_states, scale: float = 1.0, num_frames=1):
    def forward(self, hidden_states, num_frames=1, temp_compress=False):
        ...
        if self.add_temp_downsample and temp_compress:
            hidden_states = self.temp_convs_down(hidden_states, num_frames=num_frames)
