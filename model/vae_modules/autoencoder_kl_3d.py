...
from model.vae_modules.vae_3d import Decoder3D, Encoder
...
class AutoencoderKL_3D(ModelMixin, ConfigMixin, FromOriginalVAEMixin):
    r"""
    ...
    def encode(
        ...
        generator: Optional[torch.Generator] = None, num_frames: int = 1, return_dict: bool = True
        ...
        if self.use_tiling and (x.shape[-1] > self.tile_sample_min_size or x.shape[-2] > self.tile_sample_min_size):
            return self.tiled_encode(x, sample_posterior, num_frames, generator, return_dict=return_dict)

        if self.use_slicing and x.shape[0] > 1:
            encoded_slices = [self.encoder(x_slice, num_frames=num_frames) for x_slice in x.split(1)]
            h = torch.cat(encoded_slices)
        else:
            h = self.encoder(x, num_frames=num_frames)
    ...
    def _decode(self, z: torch.FloatTensor, num_frames: int=1, return_dict: bool = True) -> Union[DecoderOutput, torch.FloatTensor]:
    ...
    @apply_forward_hook
    def decode(self, z: torch.FloatTensor, num_frames: int=1, return_dict: bool = True) -> Union[DecoderOutput, torch.FloatTensor]:
        """
        ...
        if self.use_slicing and z.shape[0] > 1:
            decoded_slices = [self._decode(z_slice, num_frames=num_frames).sample for z_slice in z.split(1)]  # batch维度拆开infer
    ...
    def tiled_encode(self, x: torch.FloatTensor, sample_posterior: bool = False,  num_frames: int =1,
        generator: Optional[torch.Generator] = None, return_dict: bool = True) -> Union[AutoencoderKLOutput, torch.FloatTensor]:
        ...
        for i in range(0, x.shape[2], overlap_size):
            ...
            for j in range(0, x.shape[3], overlap_size):
                ...
                tile = self.encoder(tile, num_frames=num_frames)
    ...
    def tiled_decode(self, z: torch.FloatTensor, num_frames: int=1, return_dict: bool = True) -> Union[DecoderOutput, torch.FloatTensor]:
        r"""
        ...
        for i in range(0, z.shape[2], overlap_size):
            ...
            for j in range(0, z.shape[3], overlap_size):
                ...
                decoded = self.decoder(tile, num_frames=num_frames)
                row.append(decoded)
            rows.append(row)
        result_rows = []
        for i, row in enumerate(rows):
    ...
    def forward(
        ...
        """
        x = sample
        enc = self.encode(x, sample_posterior=sample_posterior, generator=generator, num_frames=num_frames).sample  # class DiagonalGaussianDistribution
        dec = self.decode(enc, num_frames=num_frames).sample
